...
class Tile {
  Tile([this.x = 0, this.y = 0, this.z = 0]);
  int x;
  int y;
...
class _FlutterEarthState extends State<FlutterEarth>
  ...
  int _lastGestureTime = 0; // DateTime.now().millisecondsSinceEpoch;
  ...
  List<HashMap<int, Tile>> tiles =
      List<HashMap<int, Tile>>.empty(growable: true);
  ...
  Tile getTile(int x, int y, int z) {
    final key = (x << 32) + y;
    var tile = tiles[z].putIfAbsent(key, () => Tile(x, y, z));
    tiles[z][key] = tile;
    ...
    if (tile.status != TileStatus.ready) {
      for (int i = z; i >= 0; i--) {
        ...
        final key1 = (x1 << 32) + y1;
        final tile1 = tiles[i].putIfAbsent(key1, () => Tile(x1, y1, z));
  ...
  void initMeshTexture(Mesh mesh) {
    ...
    if (tile.status == TileStatus.ready) {
      ...
      if (tile.z != zoomLevel) {
        ...
        final scale = math.pow(2, tile.z - zoomLevel);
  ...
  Mesh initMeshFaces(Mesh mesh, int subdivisionsX, int subdivisionsY) {
    ...
    final List<Polygon> faces = List<Polygon>.filled(
        mesh.indexCount + 2 * subdivisionsX * subdivisionsY,
        Polygon(0, 0, 0, 0));
  ...
  void drawTiles(Canvas canvas, Size size) {
    ...
    final meshList = List<Mesh>.empty(growable: true);
  ...
  void animateCamera(
    ...
    final fallTime = Duration.millisecondsPerSecond *
    ...
    _lastRotationAxis = quaternionAxis(q1);
    _lastQuaternion = q0;

    final panRadians = q1.radians;
    final panTime = Duration.millisecondsPerSecond *
        (panRadians * _radius * math.pow(2, riseZoom)).abs() /
        panSpeed;

    final int duration = (riseTime + panTime + fallTime).ceil();
            ...
            if (!animController.isCompleted) {
              if (panAnimation != null) {
                final q = Quaternion.axisAngle(
                    _lastRotationAxis, panAnimation?.value ?? 0);
                quaternion = _lastQuaternion * q;
              }
              if (riseAnimation != null) {
                if (animController.value < _panCurveEnd) {
                  zoom = riseAnimation?.value ?? 0;
                }
              }
              if (zoomAnimation != null) {
                if (animController.value >= _panCurveEnd) {
                  zoom = zoomAnimation?.value ?? 0;
                }
              }
              widget.onCameraMove(position, zoom);
            } else {
